...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	switch (keycode) {
	case EMAIL:
		if (record->event.pressed) {      SEND_STRING("email"); } break;
	case PASS:
		if (record->event.pressed) {       SEND_STRING("pass"); } break;
	case PIN:
		if (record->event.pressed) {        SEND_STRING("pin"); } break;
	case TESTA:
		if (record->event.pressed) {      SEND_STRING("testa"); } break;
	case TESTB:
		if (record->event.pressed) {      SEND_STRING("testb"); } break;
	case TESTC:
		if (record->event.pressed) {      SEND_STRING("testc"); } break;
	case TESTD:
		if (record->event.pressed) { SEND_STRING("lol testie"); } break;
	}
	return true;
}